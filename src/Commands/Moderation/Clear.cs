...
#pragma warning restore IDE0045 // Convert to conditional expression
			...
			messages = filterType switch
			{
				...
				FilterType.Duplicates => FilterDuplicates(messages),
		...
		public static IEnumerable<DiscordMessage> FilterDuplicates(IEnumerable<DiscordMessage> messages) => messages.OrderBy(message => message.CreationTimestamp).GroupBy(message => message.Content).Select(group => group.First());
	}
}
