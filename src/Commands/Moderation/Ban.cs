...
namespace Tomoe.Commands.Moderation
{
	public class Ban : BaseCommandModule
	{
		...
		public async Task User(CommandContext context, [Description("The person to be banned.")] DiscordUser victim, [Description("(Optional) Removed the victim's messages from the pass `x` days.")] int pruneDays = 7, [Description("(Optional) The reason why the person is being banned."), RemainingText] string banReason = Constants.MissingReason)
		{
			...
			_ = await Program.SendMessage(context, $"{victim.Mention} has been permanently banned{(sentDm ? '.' : " (Failed to DM).")} Reason: {Formatter.BlockCode(Formatter.Strip(banReason))}", null, new UserMention(victim.Id));
		}
		...
		[Command("ban")]
		public async Task Group(CommandContext context, [Description("(Optional) The reason why the people are being banned.")] string banReason = Constants.MissingReason, [Description("The people to be banned.")] params DiscordUser[] victims) => await Group(context, default, banReason, victims);

		[Command("ban")]
		public async Task Group(CommandContext context, [Description("The people to be banned.")] params DiscordUser[] victims) => await Group(context, default, default, victims);
	}
}
