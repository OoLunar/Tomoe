    ...
    public class ReactionAdded
    {
        ...
        public static async Task Handler(DiscordClient client, MessageReactionAddEventArgs eventArgs)
        {
            ...
            foreach (Queue queue in QueueList.ToList())
            {
                ...
                if (eventArgs.User.Id == Program.Client.CurrentUser.Id)
                {
                    continue;
                }
                ...
                {
                    await eventArgs.Message.DeleteReactionAsync(eventArgs.Emoji, eventArgs.User, "Not the correct user.");
                }
                else
                {
                    ...
                    if (queue.Type == Queue.ReactionType.Confirmation)
                    {
                        ...
                        {
                            await eventArgs.Message.DeleteReactionAsync(eventArgs.Emoji, eventArgs.User, "Not the correct emoji.");
                        }
                    }
                    else if (queue.Type == Queue.ReactionType.Custom)
                    {
                        if (!queue.Emojis.Contains(eventArgs.Emoji))
                        {
                            await eventArgs.Message.DeleteReactionAsync(eventArgs.Emoji, eventArgs.User, "Not the correct emoji.");
                        }
                        else
