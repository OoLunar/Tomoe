...
namespace Tomoe.Commands.Listeners
{
	public class GuildMemberUpdated
	{
		public static async Task Handler(DiscordClient client, GuildMemberUpdateEventArgs eventArgs)
		{
			...
			{
				if (Program.Database.User.Exists(eventArgs.Guild.Id, eventArgs.Member.Id)) Program.Database.User.SetRoles(eventArgs.Guild.Id, eventArgs.Member.Id, eventArgs.RolesAfter.Select(role => role.Id).ToArray());
				else
				{
