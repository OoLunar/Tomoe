...
namespace Tomoe
{
	public class Program
	{
		...
		internal static Dictionary<ulong, int> MemberCounts = new();
		...
		public static async Task Main(string[] args)
		{
			...
			// I feel like this could be abstracted a bit, but I also feel like it'd be going over the top.
			...
			// Register converters and commands for CNext
			...
			{
				// Converters
				foreach (Type? converter in typeof(Program).Assembly.GetTypes().Where(t => t.IsAssignableTo(typeof(IArgumentConverter))))
				{
			...
			foreach (Type type in typeof(Program).Assembly.GetTypes())
			{
				foreach (MethodInfo methodInfo in type.GetMethods())
				{
					...
					{
						EventInfo? discordEvent = typeof(DiscordShardedClient).GetEvent(subscribeToEventAttribute.EventName);
						...
						EventInfo? commandsNextEvent = typeof(CommandsNextExtension).GetEvent(subscribeToEventAttribute.EventName);
						if (commandsNextEvent != null)
						{
							commandsNextEvent.AddEventHandler(commandsNextExtensions.Values.First(), Delegate.CreateDelegate(commandsNextEvent.EventHandlerType!, type, methodInfo.Name));
							continue;
						}
						throw new CustomAttributeFormatException($"Event {subscribeToEventAttribute.EventName} not found.");
					}
				}
			}
