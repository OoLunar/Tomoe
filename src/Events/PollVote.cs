	...
	public class PollVote
	{
		...
		public static async Task PollAsync(DiscordClient client, ComponentInteractionCreateEventArgs componentInteractionCreateEventArgs)
		{
			if (componentInteractionCreateEventArgs.Id.StartsWith("poll\v", StringComparison.InvariantCultureIgnoreCase))
			{
				...
				if (database == null)
				{
					await componentInteractionCreateEventArgs.Interaction.EditOriginalResponseAsync(new DiscordWebhookBuilder().WithContent("[Error]: Internal bot error, your vote was not submitted. Please try again later."));
				...
				else
				{
					...
					if (pollModel == null)
					{
						...
						if (idParts[2] != "\tview")
						{
							content += "Your vote was not submitted.";
						}
					...
					if (votedOn == null)
					{
						...
						else
						{
							...
							await componentInteractionCreateEventArgs.Interaction.EditOriginalResponseAsync(new DiscordWebhookBuilder().WithContent("Your vote has been submitted!"));
						}
					}
					else
					{
